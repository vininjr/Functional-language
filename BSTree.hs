module BSTree (BSTree, empty, isEmpty, insert, updateOrInsert,
              search, remove, myMax, myMin, inOrderWalk,
              inProfundidade,inLargura) where
...
inProfundidade :: BSTree k v -> [v]
inProfundidade Empty = []
inProfundidade (Branch key val lt rt) = inProfundidade lt ++ [val] ++ inProfundidade rt

    --Busca em Largura
inLargura :: BSTree k v -> Int -> [v]
inLargura Empty _ = []
inLargura (Branch key val Empty Empty) k = [val] 
inLargura (Branch key val Empty rt) k = [val] ++ inLargura rt (k+1)
inLargura (Branch key val lt Empty) k = [val] ++ inLargura lt (k+1)
inLargura (Branch key val lt rt) k = [val] ++ (inLargura rt (k+1)) ++ (inLargura lt (k+1))
